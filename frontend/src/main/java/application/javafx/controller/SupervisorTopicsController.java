...
public class SupervisorTopicsController {
    ...
    @FXML
    private TableColumn<Thesis, String> student;
    ...
    private final String currentSupervisorName = UserSession.getInstance().getUsername();
    ...
    private void initialize() throws URISyntaxException, IOException, InterruptedException {
        ...
        ArrayList<Thesis> supervisorThesisList = new ArrayList<>();
        for (Thesis thesis : thesisList) {
            if (thesis.getSupervisorName().equals(currentSupervisorName)) {
                supervisorThesisList.add(thesis);
            }
            System.out.println(thesis);
        }


        List<Thesis> winnerResults = supervisorThesisList.stream().toList();
    ...
    @FXML
    private void deleteAction(ActionEvent event) throws URISyntaxException, IOException, InterruptedException {
        HttpClient client = HttpClient.newHttpClient();
        HttpRequest request = HttpRequest.newBuilder()
                .uri(new URI("http://127.0.0.1:8000/theses/" + currentThesisId + "?student_name=" + currentSupervisorName))
                .header("Content-Type", "application/json")
